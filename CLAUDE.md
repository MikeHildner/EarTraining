...
Don't add a "Generated by AI" banner to these pages — Vibe mode already
implies AI involvement, so the old per-page `.ai-note` banners (and their
CSS) were removed.
...
each also gained a nav scoring icon:
