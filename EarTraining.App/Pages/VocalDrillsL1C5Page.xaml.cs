...
public partial class VocalDrillsL1C5Page : ContentPage, IAutomatableDrill
{
    ...
    private L1C5VocalDrill _drill = null!;
    private bool _answered;
    private bool _ready;

    public VocalDrillsL1C5Page()
    {
        InitializeComponent();
    ...
    private List<(int idx, L1C5VocalDrill drill)> Pool()
    {
        var group = Group;
        var pool = new List<(int, L1C5VocalDrill)>();
        for (int i = 0; i < L1C5VocalDrill.All.Count; i++)
        {
            var d = L1C5VocalDrill.All[i];
            if (group is null || d.Group == group) pool.Add((i, d));
        }
        return pool;
    }

    private List<L1C5VocalDrill> Playable()
    {
    ...
    {
        try { await PlayDrillAsync(L1C5VocalDrill.All[int.Parse(key)]); }
        catch (Exception ex) { StatusLabel.Text = "Audio error: " + ex.Message; }
    }

    private async Task PlayDrillAsync(L1C5VocalDrill drill)
    {
        // Six notes with the drill's rhythm (quarters / eighths / a whole note to finish).
