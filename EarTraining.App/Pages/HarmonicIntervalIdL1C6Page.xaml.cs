...
public partial class HarmonicIntervalIdL1C6Page : ContentPage, IAutomatableDrill
{
    ...
    private L1C6IntervalDrill _drill = null!;
    private bool _answered;
    private bool _ready;

    public HarmonicIntervalIdL1C6Page()
    {
        InitializeComponent();
    ...
    private List<(int idx, L1C6IntervalDrill drill)> Pool()
    {
        var group = Group;
        var pool = new List<(int, L1C6IntervalDrill)>();
        for (int i = 0; i < L1C6IntervalDrill.Harmonic.Count; i++)
        {
            var d = L1C6IntervalDrill.Harmonic[i];
            if (group is null || d.Group == group) pool.Add((i, d));
        }
        return pool;
    }

    private List<L1C6IntervalDrill> Playable()
    {
    ...
    private string LabelFor(L1C6IntervalDrill d) => Group is null ? $"{d.Label} ({d.Group})" : d.Label;
    ...
    {
        try { await PlayDrillAsync(L1C6IntervalDrill.Harmonic[int.Parse(key)]); }
        catch (Exception ex) { StatusLabel.Text = "Audio error: " + ex.Message; }
    }

    private async Task PlayDrillAsync(L1C6IntervalDrill drill)
    {
