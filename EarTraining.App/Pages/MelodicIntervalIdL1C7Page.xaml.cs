...
public partial class MelodicIntervalIdL1C7Page : ContentPage, IAutomatableDrill
{
    ...
    public MelodicIntervalIdL1C7Page()
    {
        InitializeComponent();
    ...
    private List<(int idx, L1C7IntervalDrill drill)> Pool()
    {
        ...
        for (int i = 0; i < L1C7IntervalDrill.Melodic.Count; i++)
        {
            var d = L1C7IntervalDrill.Melodic[i];
    ...
    {
        try { await PlayDrillAsync(L1C7IntervalDrill.Melodic[int.Parse(key)]); }
        catch (Exception ex) { StatusLabel.Text = "Audio error: " + ex.Message; }
    }
