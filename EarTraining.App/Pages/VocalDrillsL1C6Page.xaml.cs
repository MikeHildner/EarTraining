...
public partial class VocalDrillsL1C6Page : ContentPage, IAutomatableDrill
{
    ...
    private L1C6VocalDrill _drill = null!;
    private bool _answered;
    private bool _ready;

    public VocalDrillsL1C6Page()
    {
        InitializeComponent();
    ...
    // "Maj 2nd", "Min 7th", or null = both
    private string? Group => SeventhsRadio.IsChecked ? "Min 7th" : BothRadio.IsChecked ? null : "Maj 2nd";

    private List<(int idx, L1C6VocalDrill drill)> Pool()
    {
        var group = Group;
        var pool = new List<(int, L1C6VocalDrill)>();
        for (int i = 0; i < L1C6VocalDrill.All.Count; i++)
        {
            var d = L1C6VocalDrill.All[i];
            if (group is null || d.Group == group) pool.Add((i, d));
        }
        return pool;
    }

    private List<L1C6VocalDrill> Playable()
    {
    ...
    {
        // The ten syllable patterns repeat across both intervals, so in "Both" mode the include
        // rows carry a (Maj 2nd) / (Min 7th) suffix to disambiguate.
    ...
    private void BuildAnswers()
    {
        ...
        foreach (var drill in Playable())
        {
            ...
            if (_answerButtons.ContainsKey(label)) continue; // dedupe shared syllable patterns
    ...
    {
        try { await PlayDrillAsync(L1C6VocalDrill.All[int.Parse(key)]); }
        catch (Exception ex) { StatusLabel.Text = "Audio error: " + ex.Message; }
    }

    private async Task PlayDrillAsync(L1C6VocalDrill drill)
    {
