...
public partial class MelodicIntervalIdL1C6Page : ContentPage, IAutomatableDrill
{
    ...
    public MelodicIntervalIdL1C6Page()
    {
        InitializeComponent();
    ...
    private List<(int idx, L1C6IntervalDrill drill)> Pool()
    {
        ...
        for (int i = 0; i < L1C6IntervalDrill.Melodic.Count; i++)
        {
            var d = L1C6IntervalDrill.Melodic[i];
    ...
    {
        try { await PlayDrillAsync(L1C6IntervalDrill.Melodic[int.Parse(key)]); }
        catch (Exception ex) { StatusLabel.Text = "Audio error: " + ex.Message; }
    }
