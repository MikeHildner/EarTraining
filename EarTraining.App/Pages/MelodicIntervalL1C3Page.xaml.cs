...
public partial class MelodicIntervalL1C3Page : ContentPage, IAutomatableDrill
{
    ...
    public MelodicIntervalL1C3Page()
    {
        InitializeComponent();
    ...
    private List<(int idx, L1C3Drill drill)> Pool()
    {
        ...
        for (int i = 0; i < L1C3Drill.Melodic.Count; i++)
        {
            var d = L1C3Drill.Melodic[i];
    ...
    {
        try { await PlayDrillAsync(L1C3Drill.Melodic[int.Parse(key)]); }
        catch (Exception ex) { StatusLabel.Text = "Audio error: " + ex.Message; }
    }
