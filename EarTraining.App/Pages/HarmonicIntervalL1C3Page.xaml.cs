...
public partial class HarmonicIntervalL1C3Page : ContentPage, IAutomatableDrill
{
    ...
    public HarmonicIntervalL1C3Page()
    {
        InitializeComponent();
    ...
    private List<(int idx, L1C3Drill drill)> Pool()
    {
        ...
        for (int i = 0; i < L1C3Drill.Harmonic.Count; i++)
        {
            var d = L1C3Drill.Harmonic[i];
    ...
    {
        try { await PlayDrillAsync(L1C3Drill.Harmonic[int.Parse(key)]); }
        catch (Exception ex) { StatusLabel.Text = "Audio error: " + ex.Message; }
    }
